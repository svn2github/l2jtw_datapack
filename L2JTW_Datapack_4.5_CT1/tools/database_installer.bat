...
echo.
echo (3) 導入全新新的資料：所有的資料將會被移除並且導入新的資料
echo.
echo (4) 查看存取的設定值
...
path|find "MySQL">NUL
if %errorlevel% == 0 (
echo 上面是找到的 MySQL 的位置，此位置將會被設為基本設定，如果想換位置請修改...
echo 如果顯示為「Not Found」，表示位置錯誤，請在下面輸入正確的位置...
set mysqlBinPath=use path
) else (
echo 無法找到 mysql 正確位置，請輸入 mysql.exe 的正確位置...
echo.
echo 如果還是不確定這到底是什麼意思和如何操作，請去相關網站查詢或者可以至 L2JTW 官方網站發問或尋找相關資訊
)
echo.
...
echo 4-其他設定
echo --------------------
set /P cmode="顏色模式 (c)為顏色 or (n)為無顏色（基本設定「%cmode%」）: "
...
echo.
echo 設定成功！
echo 你的設定值將儲存在「%config_file%」，所有的帳號密碼將以明碼顯示
echo.
echo 請按任意鍵繼續 . . .
pause> nul
goto loadconfig
...
echo.
set cstprompt=n
set /p cstprompt=安裝 custom 自訂資料表: (y) 確定 or (N) 取消 or (q) 退出？（預設值-取消）:
