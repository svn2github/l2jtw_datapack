...
public class EventItemHandler implements IItemHandler
{
	private static final Logger _log = Logger.getLogger(EventItemHandler.class.getName());
	...
	private final void useBlockCheckerItem(final L2PcInstance castor, L2ItemInstance item)
	{
		...
		if(!castor.destroyItem("BlockChecker", item.getObjectId(), 1, castor, true))
			return;
		...
		if(holder != null)
		{
			...
			for(L2PcInstance pc : castor.getKnownList().getKnownPlayersInRadius(sk.getEffectRange()))
			{
				if(holder.getPlayerTeam(pc) != team)
