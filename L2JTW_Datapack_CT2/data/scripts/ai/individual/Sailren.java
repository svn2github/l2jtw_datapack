...
public class Sailren extends L2AttackableAIScript
{
	...
	if (npc != null)
	{
		...
		this.startQuestTimer("start2",1, sailren, null);
	    }
	    if (event.equalsIgnoreCase("start"))
	    {
		this.startQuestTimer("IDLE", 500, npc, null, true);
		_LastAction = System.currentTimeMillis();
		npc.setRunning();
		npc.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, new L2CharPosition(27628,-6109,-1982,44732));
		this.startQuestTimer("mob_has_arrived",500, npc, null, true);
	    }
	    if (event.equalsIgnoreCase("start2"))
	    {
		this.startQuestTimer("IDLE", 200, npc, null, true);
		_LastAction = System.currentTimeMillis();
		npc.setIsInvul(true);
		npc.setRunning();
	    ...
	    else if (event.equalsIgnoreCase("IDLE"))
	    {
		...
		npc.setIsParalyzed(false);
		npc.setRunning();
		this.cancelQuestTimer("IDLE", npc, null);
	    }
	...
	if (GrandBossManager.getInstance().getBossStatus(SAILREN) == DORMANT || GrandBossManager.getInstance().getBossStatus(SAILREN) == WAITING)
	    ...
	    htmltext = "<html><body>席琳聖像：<br><font color=\"LEVEL\">現在還不能允許你跟賽爾蘭對戰。請回吧。</font></body></html>";
	return htmltext;
    }
	...
	if (npc.getNpcId() == VELOCIRAPTOR || npc.getNpcId() == PTEROSAUR || npc.getNpcId() == TYRANNOSAURUS)
	{
			if (getQuestTimer("mob_has_arrived", npc, null) != null)
			{
				...
				this.startQuestTimer("camera_6",1, npc, null);
			}
	}
