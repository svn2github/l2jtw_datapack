...
//  private static final int ZONE = 88888;
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
  {
    ...
    if (npc.getNpcId() == ORBYU)
    {
     switch (st.getState())
      {
      // Vytvoření questu pokud hráč splňuje podmínku min. lvlu 75 + kompletní quest 10271_TheEnvelopingDarkness
			case State.CREATED:
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (st == null)
			return htmltext;
		// Quest přijmut + zvuk přijmutí questu
       ...
       // Quest update + zvuk updatu questu
	...
	{
   // Při kondici 5 se budou dropovat quest itemy
		final QuestState st = player.getQuestState(qn);
		if (st.getInt("cond") == 5)
		{
			final long count = st.getQuestItemsCount(FRAGMENT_POWDER);
			if (count < 100)
			{  // Rate quest dropu ovliňuje config
				...
				if (numItems > 0)
				{ 
         // Při 100x FRAGMENT_POWDER se už nebudou dropovat další quest itemy
					if (count + numItems >= 100)
					{
						numItems = 100 - (int)count;
						st.playSound("ItemSound.quest_middle");
					}
         // Pokud hráč nemá 100x FRAGMENT_POWDER dropují se itemy dál
					else
